...
#テスト
values::Array{Float64} = [1.0, -124.0, 1.0]
...
println("===解の公式を求める===")
approximation_value::Array{Float64} = approximation_func(values[1], values[2], values[3])
println(approximation_value)

#正確な解を求める関数
#f(x) = a*x^2 + b*x + c = 0
function true_func(c::Float64, b::Float64, a::Float64)
    coefficient::Array{Float64} = [c, b, a]
    return roots(Polynomial(coefficient))
end

println("===正確な解を求める===")
true_value::Array{Float64} = true_func(values[1], values[2], values[3])
println(true_value)

#絶対誤差を求める関数
function absolute_error_func(true_value::Array{Float64}, approximation_value::Array{Float64})
    return [
...
println("===絶対誤差===")
absolute_error::Array{Float64} = absolute_error_func(true_value, approximation_value)
println(absolute_error)

#相対誤差を求める関数
function relative_error_func(true_value::Array{Float64}, approximation_value::Array{Float64})
    return [
...
println("===相対誤差===")
relative_error::Array{Float64} = relative_error_func(true_value, approximation_value)
...
#残差を求める関数
function rest_func(c::Float64, b::Float64, a::Float64, true_value::Array{Float64}, approximation_value::Array{Float64})
    return [
...
println("==========桁落ちを防いだパターン==========")
prevent_digit_loss::Array{Float64} = prevent_digit_loss_func(values[1], values[2], values[3])
println(prevent_digit_loss)

println("===絶対誤差===")
absolute_error::Array{Float64} = absolute_error_func(true_value, prevent_digit_loss)
println(absolute_error)

println("===相対誤差===")
relative_error::Array{Float64} = relative_error_func(true_value, prevent_digit_loss)
println(relative_error)
